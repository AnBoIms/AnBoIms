...
def adjust_polygon(original_polygon, margin=20):
    # 좌표 순서 변경
    reordered_polygon = [
        original_polygon[0],  
        original_polygon[3],  
    ...
    # numpy 배열로 변환하고 margin 적용
    reordered_polygon = np.array(reordered_polygon, dtype=np.float32)
    reordered_polygon += np.array([
        [-margin, -margin], 
        [-margin, margin],   
        [margin, margin],    
        [margin, -margin]    
